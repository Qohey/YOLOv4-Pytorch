...
class Tester:
    ...
    def test(self, conf_thresh=0.6, iou_thresh=0.6):
        ...
        seen = 0
        logger.info(("%12s" * 8) % ("Class", "Images", "Targets", "P", "R", "mAP@.5", "mAP@.5:.95", ""))
        pbar = tqdm(enumerate(self.data_loader), total=len(self.data_loader), unit="img")
        for batch_i, (img, targets, paths, shapes) in pbar:
            ...
            for si, pred in enumerate(output):
                ...
                if num_labels:
                    ...
                    for cls in torch.unique(target_class_tensor):
                        target_indeces = (cls == target_class_tensor).nonzero(as_tuple=False).view(-1)  # Target indices
                        predict_indeces = (cls == pred[:, 5]).nonzero(as_tuple=False).view(-1)          # Prediction indices
            ...
            tmp_stats = deepcopy([np.concatenate(x, 0) for x in zip(*stats)])
            if len(tmp_stats) and tmp_stats[0].any():
                p, r, ap, f1, ap_class = ap_per_class(*tmp_stats)
                p, r, ap50, ap = p[:, 0], r[:, 0], ap[:, 0], ap.mean(1)
                mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
                nt = np.bincount(tmp_stats[3].astype(np.int64), minlength=self.num_class)
            else:
                nt = torch.zeros(1)
            del tmp_stats

            info = ("%12s" + "%12.3g" * 6 + "%12s") % ("all", seen, nt.sum(), mp, mr, map50, map, "")
